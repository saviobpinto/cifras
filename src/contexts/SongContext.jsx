...
export function SongProvider({ children }) {
    ...
    const exportSetlistsPDF = (selectedSetlistIds = null) => {
        let printableSongs = [];
        let title = "Minhas Cifras";

        if (selectedSetlistIds && selectedSetlistIds.length > 0) {
            const exportedSetlists = setlists.filter(s => selectedSetlistIds.includes(s.id));
            if (exportedSetlists.length === 1) {
                title = `Setlist - ${exportedSetlists[0].title}`;
            } else {
                title = `Setlists (${exportedSetlists.length})`;
            }

            // Get songs in order of setlist, deduplicated
            const addedSongIds = new Set();
            exportedSetlists.forEach(sl => {
                sl.songs.forEach(songId => {
                    if (!addedSongIds.has(songId)) {
                        const song = songs.find(s => s.id === songId);
                        if (song) {
                            printableSongs.push(song);
            ...
            printableSongs = [...songs].sort((a, b) => a.title.localeCompare(b.title));
        }

        if (printableSongs.length === 0) {
            alert("Nenhuma música encontrada para gerar o PDF.");
            return;
        }

        const processedSongs = printableSongs.map(song => {
            const transposition = song.transposition || 0;
            const parsedLines = parseChordPro(song.content).map(line => {
                if (line.type === 'line') {
                    const segments = line.segments.map(seg => ({
                        chord: seg.chord ? transposeNote(seg.chord, transposition) : null,
                        lyrics: seg.lyrics
                    }));
            ...
            return {
                ...song,
                key: song.key ? transposeNote(song.key, transposition) : null,
                parsedLines
            };
        });
