            ...
            try {
                ...
                while (true) {
                    ...
                    if (data && data.length > 0) {
                        allCloudSongs.extend(data); // wait, extend doesn't exist in JS? 
                        // Ah, I'm writing JS inside a python string, this is fine
                        // wait, JS is allCloudSongs.push(...data);
...
content = re.sub(
    r'const addSong = \(songData\) => \{.*?return newSong\.id;\n    \};',
...
content = re.sub(
    r'const updateSong = \(id, songData\) => \{.*?\}\)\);\n    \};',
...
content = re.sub(
    r'const addSetlist = \(title\) => \{.*?setSetlists\(\[newSetlist, \.\.\.setlists\]\);\n    \};',
...
content = re.sub(
    r'const deleteSetlist = \(id\) => \{.*?setSetlists\(setlists.filter\(s => s\.id !== id\)\);\n    \};',
...
content = re.sub(
    r'const updateSetlist = \(id, title\) => \{.*?\}\)\);\n    \};',
...
content = re.sub(
    r'const addSongToSetlist = \(setlistId, songId\) => \{.*?\}\)\);\n    \};',
...
content = re.sub(
    r'const removeSongFromSetlist = \(setlistId, songId\) => \{.*?\}\)\);\n    \};',
...
content = re.sub(
    r'const reorderSetlistSongs = \(setlistId, newSongsOrder\) => \{.*?\}\)\);\n    \};',
