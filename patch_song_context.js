const fs = require('fs');
const file = '/Users/saviobpinto/Documents/workspace/cifras/src/contexts/SongContext.jsx';
...
const fetchFromCloudRegex = /useEffect\(\(\) => \{\n\s*async function fetchFromCloud\(\) \{[\s\S]*?\}, \[isLoaded, session\]\);/m;
const newFetchLogic = `
...
`;
content = content.replace(fetchFromCloudRegex, newFetchLogic.trim());

const syncTimeoutRegex = /const syncTimeout = useRef\(null\);\n\s*useEffect\(\(\) => \{[\s\S]*?\}, \[songs, setlists, isLoaded, isCloudSynced, session\]\);/m;
const newSyncTimeout = `
...
`;
content = content.replace(syncTimeoutRegex, newSyncTimeout.trim());

// Replacements for mutations
content = content.replace(
    /const addSong = \(songData\) => \{[\s\S]*?return newSong\.id;\n    \};/m,
...
content = content.replace(
    /const updateSong = \(id, songData\) => \{[\s\S]*?\}\)\);\n    \};/m,
...
content = content.replace(
    /const addSetlist = \(title\) => \{[\s\S]*?setSetlists\(\[newSetlist, \.\.\.setlists\]\);\n    \};/m,
...
content = content.replace(
    /const deleteSetlist = \(id\) => \{[\s\S]*?setSetlists\(setlists.filter\(s => s\.id !== id\)\);\n    \};/m,
...
content = content.replace(
    /const updateSetlist = \(id, title\) => \{[\s\S]*?\}\)\);\n    \};/m,
...
content = content.replace(
    /const addSongToSetlist = \(setlistId, songId\) => \{[\s\S]*?\}\)\);\n    \};/m,
    `const addSongToSetlist = (setlistId, songId) => {
        setSetlists(setlists.map(s => {
            if (s.id === setlistId && !s.songs.includes(songId)) {
...
content = content.replace(
    /const removeSongFromSetlist = \(setlistId, songId\) => \{[\s\S]*?\}\)\);\n    \};/m,
...
content = content.replace(
    /const reorderSetlistSongs = \(setlistId, newSongsOrder\) => \{[\s\S]*?\}\)\);\n    \};/m,
    `const reorderSetlistSongs = (setlistId, newSongsOrder) => {
        setSetlists(setlists.map(s => {
            if (s.id === setlistId) {
                const updated = { ...s, songs: newSongsOrder };
                syncRowToCloud('cifras_setlists', setlistId, updated, false);
                return updated;
            }
            return s;
